...
class MagickWandError(Exception):
    def __init__(self, severity, desc):
        super(MagickWand, self).__init__('{}: {}'.format(severity, desc))
...
class MagickWand():
    ...
    def _raise_error(self):
        ...
        desc_ptr = _get_exception(self._wand, severity)
        try:
            desc = unpack('s', desc_ptr)
        finally:
            desc_ptr = _relinquish_memory(desc_ptr)
